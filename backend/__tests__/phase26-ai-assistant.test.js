 ...
 *
 * Verifies:
 * 1. AI Service unit tests (prompt sanitization, structured output compliance, deterministic fallback engine, date calculations).
 * 2. POST /ai/generate-task API endpoint execution with authentication, validation, and project context.
 * 3. Rate limiting and input guards.
 * 4. Multi-tenant team scoping & project security.
 * 5. Atomic task creation with initial AI-generated subtasks via POST /tasks.
 */
...
describe('Phase 26 — AI Task Assistant', () => {
  ...
  describe('2. POST /ai/generate-task API Execution', () => {
    ...
    it('returns 404 if projectId belongs to another team', async () => {
      // Alice (Team A) attempts to pass Bob's project (Team B)
      const res = await request(app)
        .post('/ai/generate-task')
        .set('Authorization', `Bearer ${tokenA}`)
        .send({
