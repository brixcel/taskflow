...
const { emitTaskUpdated } = require('../services/realtime');

const router = express.Router({ mergeParams: true }); // inherit :taskId from parent when mounted under /tasks/:taskId/subtasks
...
async function requireTaskInTeam(req, res, taskId) {
  const targetTaskId = taskId || req.params.taskId || req.params.id;
  if (!targetTaskId) {
...
// ─── POST /tasks/:taskId/subtasks — create a single subtask ────────────────────
...
router.post('/', validate(schemas.subtaskCreate), async (req, res) => {
  try {
    ...
    emitTaskUpdated(req.teamId, { id: task.id });
...
// ─── POST /tasks/:taskId/subtasks/batch — batch create subtasks (AI breakdown) ─

router.post('/batch', validate(schemas.subtasksBatchCreate), async (req, res) => {
  try {
    const task = await requireTaskInTeam(req, res);
    if (!task) return;

    const { subtasks: items } = req.body;
    ...
    let baseOrder = (lastSubtask?.order ?? 0);

    const createdSubtasks = await prisma.$transaction(
      items.map((item, idx) => {
        const itemOrder = typeof item.order === 'number' ? item.order : baseOrder + (idx + 1) * 1000;
    ...
    await prisma.activity.create({
      data: {
        ...
        userId: req.userId,
        action: 'subtask_created',
        details: `Added ${createdSubtasks.length} subtask(s) via AI breakdown`,
      },
    });
    ...
    if (logger && logger.error) {
      logger.error({ err: error }, 'POST /tasks/:taskId/subtasks/batch failed');
    }
    res.status(500).json({ error: 'Something went wrong creating subtasks batch' });
  }
});
...
router.patch('/:subtaskId', validate(schemas.subtaskUpdate), async (req, res) => {
  try {
    ...
    emitTaskUpdated(req.teamId, { id: existingSubtask.taskId });
